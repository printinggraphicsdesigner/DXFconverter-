...
Perfect Curves with ezdxf.path - Lectra Compatible
"""
VERSION = "v2.0"
    ...
    def _parse_block(self, block):
        ...
        for e in block:
            if e.dxftype()=="TEXT" and re.match(r"^#\s*\d+$", e.dxf.text.strip()) and str(e.dxf.layer)=="1":
                p = e.dxf.insert
                rules[(round(p.x*sc, 6), round(p.y*sc, 6))] = int(re.search(r"\d+", e.dxf.text).group())
        
        for e in block:
            t, lay = e.dxftype(), str(getattr(e.dxf, "layer", "1"))
            if t == "POLYLINE":
                pts = [(round(v.dxf.location.x*sc, 6), round(v.dxf.location.y*sc, 6)) for v in e.vertices]
                if e.is_closed and pts and pts[0]!=pts[-1]: pts.append(pts[0])
                if pts:
                    self._add("POLYLINE", pts, lay)
                    if lay=="1":
                        self._base_cut_polys.append(pts)
                        gi = [(i, rules[(round(p[0],6), round(p[1],6))]) for i,p in enumerate(pts[:-1]) if (round(p[0],6), round(p[1],6)) in rules]
                        if gi and not self.grade_indices: self.grade_indices = gi
            elif t == "LWPOLYLINE":
                try:
                    path = make_path(e)
                    pts = [(round(p[0]*sc, 6), round(p[1]*sc, 6)) for p in path.flattening(distance=0.005)]
                    if pts: self._add("LWPOLYLINE", pts, lay)
                except:
                    pts = [(round(p[0]*sc, 6), round(p[1]*sc, 6)) for p in e.get_points()]
                ...
                self._add("LINE", [(round(s.x*sc, 6), round(s.y*sc, 6)), (round(e2.x*sc, 6), round(e2.y*sc, 6))], lay)
            elif t == "ARC":
                try:
                    path = make_path(e)
                    pts = [(round(p[0]*sc, 6), round(p[1]*sc, 6)) for p in path.flattening(distance=0.005)]
                ...
                try:
                    ...
                    if txt and not re.match(r"^#\s*\d+$", txt):
                        ...
                        self._add("TEXT", [(round(px, 6), round(py, 6))], lay, text=txt, height=h)
                except: pass
