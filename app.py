...
Exact Desktop App Logic Port - Full Features
"""
VERSION = "v2.0"
...
class ActualSizePDFExporter:
    ...
    def export(self, parser, out_path):
        ...
        self._export_flat(parser, out_path)

    def _export_flat(self, parser, out_path):
        b = parser.bounds
        pad = self.MARGIN
        ox = -b[0]+pad
        oy = -b[1]+pad
        ...
        c.setSubject("Do Not Scale – All Sizes")
        c.setCreator(f"Lectra Converter {VERSION}")
        ...
        if parser.graded_polys and parser.rul_parser:
            ...
            for ent in parser.entities:
                ...
                if lay in ("7", "1"):
                    continue
                if ent["type"] == "TEXT" or len(ent["points"]) < 2:
                    continue
                sw = self.LAYER_STROKE.get(lay, 0.25)
                col = ent.get("color", (100, 100, 100))
                c.setLineWidth(sw)
                c.setStrokeColorRGB(col[0]/255, col[1]/255, col[2]/255)
                pts = ent["points"]
                p = c.beginPath()
            ...
            for ent in parser.entities:
                ...
                etype = ent["type"]
                pts = ent["points"]
                sw = self.LAYER_STROKE.get(lay, 0.3)
                c.setLineWidth(sw)
                c.setStrokeColorRGB(0, 0, 0)
                if etype == "TEXT":
                    if pts:
                        h_pt = max(ent.get("height", 0.25)*cm*0.8, 4)
                        c.setFont("Helvetica", min(h_pt, 8))
...
class SVGExporter:
    ...
    def _export_graded(self, parser, out_path):
        ...
        for si, sname in enumerate(sizes):
            ...
            for poly in polys:
                ...
                lines.append(f'          fill="none" stroke="{col}" stroke-width="{sw}"/>')
                ...
                lines.append(f'          font-weight="bold" font-size="7" fill="{col}">')
                lines.append(f'      Size: {sname}')
                lines.append(f'    </text>')
        ...
        for ent in parser.entities:
            ...
            col_ent = {"14": "#50DC50", "8": "#C8A000", "4": "#FF6464"}.get(lay, "#808080")
            d = f"M {sx(pts_mm[0][0])},{sy(pts_mm[0][1])}"
            for pt in pts_mm[1:]:
                d += f" L {sx(pt[0])},{sy(pt[1])}"
            other_lines.append(f'     <path d="{d}" fill="none" stroke="{col_ent}" stroke-width="0.2"/>')
        
        if other_lines:
            lines.append('   <g id="Sew_Grain_Lines">')
    ...
    def _export_base(self, parser, out_path):
        ...
        for ent in parser.entities:
            ...
            for p in pts[1:]:
                d += f" L {sx(p[0])},{sy(p[1])}"
            col = {"1": "#000000", "14": "#50DC50", "8": "#C8A000", "4": "#FF6464"}.get(lay, "#808080")
            sw = {"1": "0.5", "14": "0.25", "8": "0.3", "4": "0.5"}.get(lay, "0.25")
    ...
    def _svg_header(self, svg_w, svg_h, bounds):
        ...
        return [
            ...
            f'  <!-- Lectra Pattern Converter {VERSION} — Actual Size 1:1 -->',
