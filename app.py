...
CLO-Compatible Output - Clean Curves, Accurate Shapes
"""
VERSION = "v5.0"
...
# Colors
LAYER_COLORS = {"1": (255,255,255), "14": (100,220,100), "8": (255,200,60), "4": (255,120,120)}
...
class AAMAParser:
    ...
    def _parse_block(self, block):
        ...
        for e in block:
            ...
            if t == "POLYLINE":
                ...
                try:
                    pts = [(p[0]*sc, p[1]*sc) for p in e.flattening(0.01)]
    ...
    def _add(self, etype, pts, layer):
        self.entities.append({"type": etype, "points": pts, "layer": layer, "color": LAYER_COLORS.get(layer, DEFAULT_COLOR)})
                ...
                for poly in parser.graded_polys.get(sn, []):
                    ...
                    for i in range(len(sc2)-1):
                        if all(0 <= c <= cw if i==0 else 0 <= c <= ch for c in [sc2[i][0], sc2[i][1], sc2[i+1][0], sc2[i+1][1]]):
                            draw.line([sc2[i], sc2[i+1]], fill=col, width=2 if sn==parser.rul_parser.sample else 1)
        for ent in parser.entities:
            pts, col = ent["points"], ent.get("color", DEFAULT_COLOR)
            if len(pts) < 2: continue
            sc2 = [(int(tx(p[0])), int(ty(p[1]))) for p in pts]
            for i in range(len(sc2)-1):
                if all(0 <= c <= cw if i==0 else 0 <= c <= ch for c in [sc2[i][0], sc2[i][1], sc2[i+1][0], sc2[i+1][1]]):
                    draw.line([sc2[i], sc2[i+1]], fill=col, width=1)
        return img
...
class PDFExporter:
    def export(self, parser, out_path):
        ...
        for ent in parser.entities:
            ...
            pts = ent["points"]
            c.setLineWidth(0.3); c.setStrokeColorRGB(0,0,0)
            p = c.beginPath()
...
def upload():
    try:
        ...
        file.save(filepath)
        parser = AAMAParser(); parser.parse(filepath)
...
def upload_rul():
    try:
        ...
        filepath = os.path.join(app.config['UPLOAD_FOLDER'], file.filename)
        file.save(filepath)
        rul = RULParser(); rul.parse(filepath)
